...
var DEFAULT_AGENT = util.format('%s/v%s wreck/v%s node.js/%s', pkg.name, pkg.version, require('wreck/package').version, process.version);
...
function UriHttpReader(uri, options) {
  ...
  var fetchMethod = this.probe ? 'HEAD' : 'GET';

  // TODO: handle case in header names
  var headers = Hoek.applyToDefaults(defaults, options.headers || {}, true);
  if ('range' in headers)
  ...
  function fetchHttp(start) {
    ...
    var location = uri;
    var onRedirect = function(statusCode, newLocation, newReq) {
    ...
    var size = -1;
    var req = Wreck.request(fetchMethod, uri, { headers: headers, agent: agent, timeout: self.timeout, redirects: 10, redirected: onRedirect }, function(err, res) {
      if (err) {
      ...
      var filesize = (size >= 0) ? start + size - skip - offset : -1;
      ...
      var meta = { url: location, mime: mimetype, size: filesize, modified: modified };
      if (self.meta) {
        // ignore change from unknown to know size
        if (self.meta.size === -1)
          meta.size = self.meta.size;

        if (!Hoek.deepEqual(self.meta, meta)) {
          tries = 0;
